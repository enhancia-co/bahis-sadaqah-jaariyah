...
export class AuthController {
    ...
    async login(req: Request, res: Response) {
        try {
            ...
            if(!username || !password) throw new HttpError(HTTP_MESSAGE.BAD_REQUEST, HTTP_STATUS.BAD_REQUEST);
            
            console.log(username, password)

            const { accessToken, user } = await this._authService.login(username, password);
    ...
    async signup(req: Request, res: Response) {
        try {
            ...
            if(!name || !username || !password) throw new HttpError(HTTP_MESSAGE.BAD_REQUEST, HTTP_STATUS.BAD_REQUEST);
            
            console.log(req.body);

            const { accessToken, user } = await this._authService.signup(name, username, password);
